---
name: book-rules-refactoring-guru
description: "Coding agent rules distilled from 'refactoring-guru'. Use when user invokes $book-rules-refactoring-guru for refactoring, code review, or design work in this domain. Refactoring.Guru — code smell catalog, refactoring techniques, diagnose-smell → choose-treatment → preserve-behavior."
version: 1.0.0
author: ciembor (agent-rules-books) + Hermes port
license: MIT
platforms: [linux, macos, windows]
source: https://github.com/ciembor/agent-rules-books
source_file: refactoring-guru/refactoring-guru.md
metadata:
  hermes:
    tags: [book-rules, code-quality, refactoring, refactoring-guru]
    homepage: https://github.com/ciembor/agent-rules-books
---

# Book Rules: Refactoring Guru

Full rule set from the original **refactoring-guru** by `ciembor/agent-rules-books`. This is the canonical `full` version (not `mini`, not `nano`) — all rules verbatim from the upstream source.

## When to use this skill

Load this skill with `$book-rules-refactoring-guru` when:

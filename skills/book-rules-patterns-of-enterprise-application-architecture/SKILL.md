---
name: book-rules-patterns-of-enterprise-application-architecture
description: "Coding agent rules distilled from 'patterns-of-enterprise-application-architecture'. Use when user invokes $book-rules-patterns-of-enterprise-application-architecture for refactoring, code review, or design work in this domain. Patterns of Enterprise Application Architecture (M. Fowler) — layers, repository, unit of work, identity map, DTO."
version: 1.0.0
author: ciembor (agent-rules-books) + Hermes port
license: MIT
platforms: [linux, macos, windows]
source: https://github.com/ciembor/agent-rules-books
source_file: patterns-of-enterprise-application-architecture/patterns-of-enterprise-application-architecture.md
metadata:
  hermes:
    tags: [book-rules, code-quality, refactoring, patterns-of-enterprise-application-architecture]
    homepage: https://github.com/ciembor/agent-rules-books
---

# Book Rules: Patterns Of Enterprise Application Architecture

Full rule set from the original **patterns-of-enterprise-application-architecture** by `ciembor/agent-rules-books`. This is the canonical `full` version (not `mini`, not `nano`) — all rules verbatim from the upstream source.

## When to use this skill

Load this skill with `$book-rules-patterns-of-enterprise-application-architecture` when:
- Refactoring code in this domain
- Reviewing or auditing code for the book's principles
- Designing new code that should follow this book's patterns
- The user explicitly invokes the book name

## Rules (full set)

The complete rule set follows. Apply ALL of them when this skill is active. If any rule conflicts with a more specific user instruction in the conversation, the user instruction wins.

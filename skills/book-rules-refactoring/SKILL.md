---
name: book-rules-refactoring
description: "Coding agent rules distilled from 'refactoring'. Use when user invokes $book-rules-refactoring for refactoring, code review, or design work in this domain. Refactoring (M. Fowler) — safe restructuring, small steps, code smells, separate refactor from feature work."
version: 1.0.0
author: ciembor (agent-rules-books) + Hermes port
license: MIT
platforms: [linux, macos, windows]
source: https://github.com/ciembor/agent-rules-books
source_file: refactoring/refactoring.md
metadata:
  hermes:
    tags: [book-rules, code-quality, refactoring, refactoring]
    homepage: https://github.com/ciembor/agent-rules-books
---

# Book Rules: Refactoring

Full rule set from the original **refactoring** by `ciembor/agent-rules-books`. This is the canonical `full` version (not `mini`, not `nano`) — all rules verbatim from the upstream source.

## When to use this skill

Load this skill with `$book-rules-refactoring` when:

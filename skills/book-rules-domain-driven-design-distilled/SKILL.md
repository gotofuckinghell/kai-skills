---
name: book-rules-domain-driven-design-distilled
description: "Coding agent rules distilled from 'domain-driven-design-distilled'. Use when user invokes $book-rules-domain-driven-design-distilled for refactoring, code review, or design work in this domain. DDD Distilled (V. Vernon) — pragmatic DDD: subdomains, contexts, context mapping, basic tactical patterns."
version: 1.0.0
author: ciembor (agent-rules-books) + Hermes port
license: MIT
platforms: [linux, macos, windows]
source: https://github.com/ciembor/agent-rules-books
source_file: domain-driven-design-distilled/domain-driven-design-distilled.md
metadata:
  hermes:
    tags: [book-rules, code-quality, refactoring, domain-driven-design-distilled]
    homepage: https://github.com/ciembor/agent-rules-books
---

# Book Rules: Domain Driven Design Distilled

Full rule set from the original **domain-driven-design-distilled** by `ciembor/agent-rules-books`. This is the canonical `full` version (not `mini`, not `nano`) — all rules verbatim from the upstream source.

## When to use this skill

Load this skill with `$book-rules-domain-driven-design-distilled` when:

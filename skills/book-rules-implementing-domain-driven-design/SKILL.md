---
name: book-rules-implementing-domain-driven-design
description: "Coding agent rules distilled from 'implementing-domain-driven-design'. Use when user invokes $book-rules-implementing-domain-driven-design for refactoring, code review, or design work in this domain. Implementing DDD (V. Vernon) — real-world DDD: aggregates, domain events, application architecture."
version: 1.0.0
author: ciembor (agent-rules-books) + Hermes port
license: MIT
platforms: [linux, macos, windows]
source: https://github.com/ciembor/agent-rules-books
source_file: implementing-domain-driven-design/implementing-domain-driven-design.md
metadata:
  hermes:
    tags: [book-rules, code-quality, refactoring, implementing-domain-driven-design]
    homepage: https://github.com/ciembor/agent-rules-books
---

# Book Rules: Implementing Domain Driven Design

Full rule set from the original **implementing-domain-driven-design** by `ciembor/agent-rules-books`. This is the canonical `full` version (not `mini`, not `nano`) — all rules verbatim from the upstream source.

## When to use this skill

Load this skill with `$book-rules-implementing-domain-driven-design` when:

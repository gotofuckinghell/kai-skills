---
name: book-rules-clean-architecture
description: "Coding agent rules distilled from 'clean-architecture'. Use when user invokes $book-rules-clean-architecture for refactoring, code review, or design work in this domain. Clean Architecture (R.C. Martin) — stable boundaries, dependency rule, business policies separated from frameworks/DB/UI."
version: 1.0.0
author: ciembor (agent-rules-books) + Hermes port
license: MIT
platforms: [linux, macos, windows]
source: https://github.com/ciembor/agent-rules-books
source_file: clean-architecture/clean-architecture.md
metadata:
  hermes:
    tags: [book-rules, code-quality, refactoring, clean-architecture]
    homepage: https://github.com/ciembor/agent-rules-books
---

# Book Rules: Clean Architecture

Full rule set from the original **clean-architecture** by `ciembor/agent-rules-books`. This is the canonical `full` version (not `mini`, not `nano`) — all rules verbatim from the upstream source.

## When to use this skill

Load this skill with `$book-rules-clean-architecture` when:

---
name: book-rules-release-it
description: "Coding agent rules distilled from 'release-it'. Use when user invokes $book-rules-release-it for refactoring, code review, or design work in this domain. Release It! (Nygard) — production survival: failures, timeouts, retries, circuit breakers, bulkheads, backpressure."
version: 1.0.0
author: ciembor (agent-rules-books) + Hermes port
license: MIT
platforms: [linux, macos, windows]
source: https://github.com/ciembor/agent-rules-books
source_file: release-it/release-it.md
metadata:
  hermes:
    tags: [book-rules, code-quality, refactoring, release-it]
    homepage: https://github.com/ciembor/agent-rules-books
---

# Book Rules: Release It

Full rule set from the original **release-it** by `ciembor/agent-rules-books`. This is the canonical `full` version (not `mini`, not `nano`) — all rules verbatim from the upstream source.

## When to use this skill

Load this skill with `$book-rules-release-it` when:

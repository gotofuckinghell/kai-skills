---
name: book-rules-the-pragmatic-programmer
description: "Coding agent rules distilled from 'the-pragmatic-programmer'. Use when user invokes $book-rules-the-pragmatic-programmer for refactoring, code review, or design work in this domain. The Pragmatic Programmer (Hunt/Thomas) — DRY at knowledge level, orthogonality, automation, fast feedback, adaptability."
version: 1.0.0
author: ciembor (agent-rules-books) + Hermes port
license: MIT
platforms: [linux, macos, windows]
source: https://github.com/ciembor/agent-rules-books
source_file: the-pragmatic-programmer/the-pragmatic-programmer.md
metadata:
  hermes:
    tags: [book-rules, code-quality, refactoring, the-pragmatic-programmer]
    homepage: https://github.com/ciembor/agent-rules-books
---

# Book Rules: The Pragmatic Programmer

Full rule set from the original **the-pragmatic-programmer** by `ciembor/agent-rules-books`. This is the canonical `full` version (not `mini`, not `nano`) — all rules verbatim from the upstream source.

## When to use this skill

Load this skill with `$book-rules-the-pragmatic-programmer` when:

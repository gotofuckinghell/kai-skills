---
name: book-rules-working-effectively-with-legacy-code
description: "Coding agent rules distilled from 'working-effectively-with-legacy-code'. Use when user invokes $book-rules-working-effectively-with-legacy-code for refactoring, code review, or design work in this domain. Working Effectively with Legacy Code (Feathers) — characterization tests, seams, sprout/wrap methods, incremental risk reduction."
version: 1.0.0
author: ciembor (agent-rules-books) + Hermes port
license: MIT
platforms: [linux, macos, windows]
source: https://github.com/ciembor/agent-rules-books
source_file: working-effectively-with-legacy-code/working-effectively-with-legacy-code.md
metadata:
  hermes:
    tags: [book-rules, code-quality, refactoring, working-effectively-with-legacy-code]
    homepage: https://github.com/ciembor/agent-rules-books
---

# Book Rules: Working Effectively With Legacy Code

Full rule set from the original **working-effectively-with-legacy-code** by `ciembor/agent-rules-books`. This is the canonical `full` version (not `mini`, not `nano`) — all rules verbatim from the upstream source.

## When to use this skill

Load this skill with `$book-rules-working-effectively-with-legacy-code` when:
